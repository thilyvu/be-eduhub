...
const {scoreCreateSchema,scoreUpdateSchema} = require('../helper/validation_score')
class APIfeatures {
...
const getListScore = async (req, res)=>{
  try {
    ...
    const features = new APIfeatures(Score.find({"createBy" :req.user._id }), req.query)
...
const getListExerciseScore = async (req, res)=>{
  try {
    ...
    const features = new APIfeatures(Score.find({"exerciseId" :req.params.exerciseId }), req.query)
      .filtering().sorting().paginating()
...
const getScoreById = async (req, res) => { 
  try {
    ...
    if(!likeScore) return res.status(400).json({msg: "Score does not exist."})

...
const {calendarCreateSchema,calendarUpdateSchema,calendarCreateClassSchema} = require('../helper/validation_calendar')
class APIfeatures {
...
const getListCalendar = async (req, res)=>{
  try {
    ...
    const features = new APIfeatures(Calendar.find({"createBy" :req.user._id }), req.query)
...
const getListClassCalendar = async (req, res)=>{
  try {
    ...
    const features = new APIfeatures(Calendar.find({"classId" :req.params.classId }), req.query)
      .filtering().sorting().paginating()
...
const getCalendarById = async (req, res) => { 
  try {
    ...
    if(!likeCalendar) return res.status(400).json({msg: "calendar does not exist."})
